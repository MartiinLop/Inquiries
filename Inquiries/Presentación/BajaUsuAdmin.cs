...
namespace Inquiries
{
    public partial class BajaUsuAdmin : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (alumnos.Length != comparar.Length)
            {
                for (int x = 0; x < alumnos.GetLength(0); x++)
                {
                    ...
                    TemplateParteAdmin.FORPanelElimUsuAdmin a = new TemplateParteAdmin.FORPanelElimUsuAdmin(alNom.Text,alApe.Text,alGru.Text, "Alumno",Convert.ToInt32(alCi.Text)) { Dock = DockStyle.Fill, TopLevel = false, TopMost = true };
            ...
            if (docentes.Length != comparar2.Length)
            {
                for (int x = 0; x < docentes.GetLength(0); x++)
                {
                    ...
                    TemplateParteAdmin.FORPanelElimUsuAdmin a = new TemplateParteAdmin.FORPanelElimUsuAdmin(dNom.Text, dApe.Text, dGru.Text, "Docente", Convert.ToInt32(dCi.Text)) { Dock = DockStyle.Fill, TopLevel = false, TopMost = true };
